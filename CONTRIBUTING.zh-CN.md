...
发布即打一个 `v` 前缀的 tag。用 `gh release create` 给当前 `HEAD` 打 tag 并推送，同时创建带自动 release notes 的 GitHub Release 页面——这会触发 `release.yml` workflow（先跑完整测试，再构建并推送 Docker 镜像到 Docker Hub `leokun123/devin-2api`，amd64 + arm64）：
